...
> 기능별 명세는 [`docs/specs/`](./specs/)에 파일로 분리 — 각 파일이 **웹 페이지 구성 → 핵심 플로우 → 백엔드 API → 완료 기준 → 테스트(최소/통합)** 구조 — 스펙만 보면 무엇이 되면 완성인지까지 알 수 있다.
...
구현 순서·진행 현황은 [PLAN.md](./PLAN.md), Phase별 실행 체크리스트는 [docs/tasks/](./tasks/README.md) 참조. 각 스펙 파일 상단의 `상태:` 줄과 「완료 기준」 체크박스로 구현 여부를 추적한다(⬜ 미착수 → 🔨 진행중 → ✅ 완료).
