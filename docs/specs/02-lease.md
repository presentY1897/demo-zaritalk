...
- [ ] 건물→호실 등록·수정·삭제가 화면에서 완주, 호실 그리드에 계약중/공실/연체 상태 색 표시
- [ ] 계약 등록 → PENDING_TENANT + 당월 청구 자동 생성, 계약 상세에 조건·연결 상태 표시
- [ ] 시드의 홍미가 번호(01055555555)로 신규 가입 → 대기 계약 노출 → 수락 → ACTIVE + 세입자 홈에 계약 표시
- [ ] 임대인 홈 대시보드 숫자가 시드 기준 정확(연체 1건 1,015,000원 / 이번 달 예정 등)

## 테스트

- **최소(단위·API)**: 호실 `@@unique([buildingId,label])` 중복 409 · 타인 건물 접근 403 · 계약 등록 시 당월 청구 생성(dueDate=납부일) · 기간 역전(start>end) 400 · 동일 호실 기간 중복 계약 409 · accept — 전화번호 불일치 403, 이미 ACTIVE 409, tenantProfileId·tenantAcceptedAt 기록 · summary 집계(연체 건수·금액, 만기 90일 필터)
- **통합(E2E)**: ①임대인 로그인 → 건물·호실·계약 등록 → 그리드에 계약중 표시 ②신규 세입자 가입 → 계약 수락 → 세입자 홈에서 납부 예정 확인
