...
- [ ] 공실 호실에서 매물 등록 → OPEN, 상태 변경(RESERVED/CLOSED) 반영
- [ ] `/search` 지도 이동·필터가 리스트와 동기화, 비로그인 접근 가능, 상세는 OG·JSON-LD 유효
- [ ] 근무지 등록 후 매물 상세 통근 조회 → 결과가 목록·핀 배지로 재사용, 외부 API 한쪽 실패에도 부분 결과 표시

## 테스트

- **최소(단위·API)**: 매물 권한 — 소유 임대인·수락 중개인만, 계약중 호실 409 · bounds 쿼리 영역 내/외 정확성·OPEN만·필터 조합 · 근무지 좌표 검증·본인 것만 CRUD · 통근(외부 mock) — ①캐시 히트 시 외부 호출 없음 ②미스 시 upsert ③한쪽 실패 → 부분 결과 저장 ④양쪽 실패 → 에러·캐시 미생성
- **통합(E2E)**: ①매물 등록 → `/search` 핀·리스트 노출 → 상세 진입 ②근무지 등록 → 상세 통근 조회 → 목록 배지 표시(키 없으면 skip)
