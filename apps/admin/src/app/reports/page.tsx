 ...
 * (환급 심사·크론 화면과 같은 이유로 `force-dynamic`).
 *
...
export default async function ReportsPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const queue = await fetchReportQueue(filter.statuses);

  return <ReportQueueView filterKey={filter.key} queue={queue} />;
}
